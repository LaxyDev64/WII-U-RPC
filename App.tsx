...
const App: Component = () => {
  ...
  return (
    <div class="text-white text-center select-none">
      <div class={`bg-sky-600 ${isMac() ? "pt-16" : "pt-6"} pb-6`}>
        <p class=" text-2xl font-semibold">[WII-U-RPC]</p>
        <FaLayers size="2x">
          <Fa icon={faToggleOn} />
        </FaLayers>
      </div>
      <div class="container pt-5 pb-5">
        ...
        <select
          id="games"
          class="bg-slate-800 border border-white rounded-lg focus:border-sky-600 w-80 h-10"
        ...
        <input
          id="status"
          class="bg-slate-800 border border-white rounded-lg focus:border-sky-600 w-80 h-10 pl-2 pr-2"
          onChange={(e) => setStatus(e.currentTarget.value)}
          placeholder="Online, Playing with Friends, etc..."
        />
      </div>
      <button
        class="rounded-xl bg-sky-700 w-20 h-10"
      ...
      <Show when={connErr()}>
        <p
          ...
          class="mt-5 font-mono underline bg-sky-600"
        >
